<script lang="ts">
    ...
    $: filtered = posts
        .filter(p => {
            // Filter by category
            const categoryMatch = selectedCategory === 'all' || p.data.category === selectedCategory;
            ...
            const searchMatch = searchTerm === '' || 
                ...
                (p.data.category && p.data.category.toLowerCase().includes(searchTerm.toLowerCase()));
...
<div class="mb-8">
    <div class="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <div class="flex items-center gap-3 ml-auto">
            <div class="w-[150px] relative">
                ...
                >
                    {#each categories as category}
                        <option value={category} class="truncate" title={category === 'all' ? 'All Categories' : category}>
            ...
            <button
                ...
                aria-label={sortOrder === 'newest' ? 'Sorted by newest first' : 'Sorted by oldest first'}
    ...
    {:else}
        {#if featuredPost}
            <div class="mb-12">
                <a href={`/blog/${featuredPost.id}/`} class="group block transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl p-2 -m-2">
                    <img src={featuredPost.data.heroImage} alt=""
                         class="w-full rounded-xl mb-4 transition-all duration-200 group-hover:shadow-lg"
                         width={1020} height={510}
                    />
                    <h2 class="text-3xl font-bold mb-2 text-gray-900 dark:text-gray-100 group-hover:text-gray-700 dark:group-hover:text-gray-200 group-hover:underline" title={featuredPost.data.title}>
                        {featuredPost.data.title}
                    </h2>
                    <div class="flex items-center gap-3 text-gray-600 dark:text-gray-400">
                        <span>{formatArticleDate(new Date(featuredPost.data.pubDate))}</span>
                        {#if featuredPost.data.category}
          <span class="inline-block text-sm bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-1 rounded" title={featuredPost.data.category}>
            {featuredPost.data.category}
          </span>
                        {/if}
                    </div>
                    <p class="mt-2 text-gray-700 dark:text-gray-300" title={featuredPost.data.description}>
        ...
        {#if filtered.length > 1}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                {#each filtered.slice(1) as post}
                    <a href={`/blog/${post.id}/`} class="group block transition-all duration-200 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg overflow-hidden">
                        <img src={post.data.heroImage} alt=""
                             class="w-full h-48 object-cover rounded-t-lg"
                             width={720} height={360}
                        />
                        <div class="p-4">
                            <h3 class="text-xl font-semibold mb-2 text-gray-900 dark:text-gray-100 group-hover:text-gray-700 dark:group-hover:text-gray-200 group-hover:underline" title={post.data.title}>
                                {post.data.title}
                            </h3>
                            <div class="flex items-center gap-3 text-gray-600 dark:text-gray-400">
                                <span>{formatPostDate(new Date(post.data.pubDate))}</span>
                                {#if post.data.category}
                  <span class="inline-block text-xs bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-1 rounded" title={post.data.category}>
                    {post.data.category}
                  </span>
                                {/if}
                            </div>
